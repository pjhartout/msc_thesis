...
\begin{frame}[fragile]{Multiple experiments with error bars}
  \begin{figure}
    ...
    \scalebox{0.75}{
      \begin{tikzpicture}[align=center, node distance=1.5cm, align=center]
        \node (coords) [bluebox] {Get \\Coordinates};
        \node (gauss) [bluebox, right of=coords, xshift=1.5cm] {Add \\gaussian noise};
        \node (cmap) [bluebox, right of=gauss, xshift=1.5cm] {Get \\contact map};
        \node (egraph) [bluebox, right of=cmap, xshift=1.5cm] {Get \\$\epsilon$-graph};
        \node (wlk) [bluebox, right of=egraph, xshift=1.5cm] {Compute \\kernel};
        \node (mmd) [bluebox, right of=wlk, xshift=1.5cm] {Compute \\MMD};
        \draw [arrow] (coords) -- (gauss);
        \draw [arrow] (gauss) -- (cmap);
        \draw [arrow] (cmap) -- (egraph);
        \draw [arrow] (egraph) -- (wlk);
...
\begin{frame}[standout]{One-sentence takeway}
  Use MMD to evaluate your protein generative protein model
\end{frame}
