...
\title{\texttt{GGNNMetrics}, a package for optimizing and understanding MMD
...
\begin{frame}[fragile]{Graph extraction from pdb file}
  ...
  \item \textcolor{blue}{Delaunay graph} (through Delaunay triangulation,
    results in denser graphs) \cite{taylor2006graph}.
  \end{itemize}

  Dependencies: try to mostly only depend on \texttt{Biopython}.
  ...
  \begin{figure}[center]
    ...
    \begin{tikzpicture}[align=center, node distance=1cm, align=center]
      ...
      \coordinate[above of=proteins] (ct);
      \node (imol) [bluebox, right of=ct, xshift=0.5cm, yshift=0.5cm] {Delaunay \\graph};
      \node (dela) [bluebox, left of=ct, xshift=-0.5cm, yshift=0.5cm] {Intra-mol.\\ graph};
      \node (contact) [orangebox, below of=proteins] {Contact map};
      \coordinate[below of=contact] (cb);
      \node (eNNG) [orangebox, right of=cb, xshift=0.5cm] {$\varepsilon$-NNG};
      \node (kNNG) [orangebox, left of=cb, xshift=-0.5cm] {$k$-NNG};
      \draw [arrow] (proteins) -- (contact);
      \draw [arrow] (contact) -- (eNNG);
      \draw [arrow] (contact) -- (kNNG);
      \draw [arrow] (proteins) -- (imol);
      \draw [arrow] (proteins) -- (dela);
    \end{tikzpicture}
...
\begin{frame}[fragile]{Evaluation}
  ...
  \item Hard coded validity check: minimal distance/distribution between CA atoms/notes, ability
    to translate this to AA sequence, ...
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Plots to understand the behaviour of MMD}
  ...
  \item Test the interpretability of the metric by checking distribution of
    distances within protein families, stable/unstable families, etc.
  \end{itemize}
\end{frame}
