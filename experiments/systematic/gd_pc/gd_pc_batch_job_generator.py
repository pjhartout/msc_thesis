...
def main(cfg):
    wl_pc = open(
        here() / "experiments/systematic/gc_pc" / "gd_pc_job_array.sh",
        "w",
    )  # write mode
    ...
    slurm_string = "srun --cpus-per-task 140 --mem-per-cpu 7G --exclude=bs-hpsvr08 poetry run python experiments/systematic/gc_pc/gc_pc.py"
