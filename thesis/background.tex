...
\section{Topological Data Analysis}\label{background:tda}
...
\section{Maximum Mean Discrepancy}\label{background:kernels}
...
samples from $\by=\{y_1,\dots, y_m\}\subseteq\mathcal{X}$, the biased estimate of $\MMD^2$ is given by:
...
\item \textbf{The degree histogram.} Given a graph $G=(E,V)$ as defined in Section
  \ref{background:graphs}, we can calculate $\deg(v)\forall v\in V$, where
...
\item \textbf{The Laplacian spectrum histogram.} The normalized graph Laplacian
  ...
  adjacency matrix (see Section \ref{background:graphs}), $I$ the identity
...
procedures described in section \ref{background:tda}, in particular the
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Three iterations of the Weisfeiler-Lehman node relabelling procedure.
