...
maximum mean discrepancy. Finally, we introduce kernels that can be used within the
...
is formed by two alpha carbons, the carboxyl group, and the hydrogen atom
...
means that the rotations of these planes enabling the 3D folding of a protein
are governed by the angle of the bonds linking the nitrogen atom to the
\textalpha{}-carbon and the other carbon atom to the \textalpha{}-carbon, named
\textphi{} and \textpsi{}. These angles' values are frequently used to validate
proteins, characterize the secondary structure of proteins (i.e. structural
features observed in certain segments of proteins), etc.
...
\begin{figure}[h]
  ...
  \caption[Schematic of the backbone of a protein.]{Schematic of the backbone of a protein. Two \textalpha{}-carbons are
    ...
    side chains of the amino acid. Image by \href{https://bio.libretexts.org/Under_Construction/Purgatory/Core_\%28Britt\%27s_page\%29/Proteins*\%23}{Marc T. Facciotti}.}
  \label{fig:backbone}
\end{figure}
...
relationship between nodes can be represented as an $n\times n$
...
labeled edges representing the distance between each pair of nodes. From there, it is
possible to either extract a $k$-nearest neighbor ($k$-NN) graph, where
$k\in\mathbb{N}>0$ defines the number of nodes directly connected to any given
node; or an $\varepsilon$-graph, where each node within a given distance
...
dissapearance (i.e. two connected components merging into one) of topological features
...
multiset of tuples, each of which contains the values at which each feature is
born or dies.
...
One such seminal method was that of generative adversarial learning, which was pioneered
...
backpropagation to minimize some appropriate loss function. A recent review of
the existing landscape of generative modeling methods has been provided by
\cite{bond2021deep}.

These techniques have been particularly successful in the image domain, where
modern \acrfull{gans} have been able to tackle multiple practical challenges
such as mode collapse\footnote{We define mode collapse as the situation when a
particular type of generated output (i.e. intra-mode outputs) lacks variety (see
also Section \ref{sec:mode_collapse_mode_drop})} and convergence failure to
produce realistic images, such as the sample seen in Figure
\ref{fig:styleganxl}. More pertinent to this thesis is the application of
generative models to graphs. The application domain has been reviewed by
\cite{zhou2020graph}. In short, graph generative networks are capable of
operating on the highly versatile and extensive graph domain. It has been shown
that they can produce small molecules as well as generate social networks, and
...
sequentially, such as GraphRNN by \cite{you2018graphrnn}, and those that
generate graphs from some latent distribution directly, such as MolGAN by
\cite{de2018molgan}.
...
also needs to be accounted for in the model structure. Furthermore, by building a
...
optimize. The last modeling-related issue when dealing with graphs is that the
presence of one edge is not independent of another, i.e. real-world graphs
...
a neural network feature computed from both real-world images and the
synthetic images. Concretely, this is achieved by calculating the squared
Wasserstein metric between the generated and real representations computed from
a neural network (commonly, the Inception v3 architecture from
\cite{szegedy2016rethinking} is used) as two multidimensional Gaussian
distributions with parameters $\mathcal{N}(\mu, \Sigma)$ and
...
For the graph domain, such a measure is infeasible due to the varying size of
...
\cite{obray2022evaluation}, \cite{thompson2022evaluation} show that using a
...
\cite{you2018graphrnn} have generally gravitated towards the \acrfull{mmd}
measure to evaluate the quality of the graph.

\section{Maximum Mean Discrepancy}\label{sec:mmd}

A significant part of this thesis is centered around investigating the \acrshort{mmd}
statistic, so we define and examine existing \acrshort{mmd} research here. Introduced by
\cite{borgwardt2006integrating} and further exposited by
\cite{gretton2012kernel}, it leverages the expressive power and
...
another. \acrshort{mmd} is therefore an ideal platform to leverage when trying to assess
generative models.

First, we define \emph{kernels}, an essential component of \acrshort{mmd}. They measure the
...
practices are discussed in more detail by \cite{obray2022evaluation}, but we want
to give an overview of possible, common, and sensible choices for descriptor,
kernel, and hyperparameter here.
...
divided all possible edges between neighbors, i.e.
  \begin{equation}
...
It is worth nothing that, none of those three descriptors take node labels into account. This is
...
\cite{obray2022evaluation}, Appendix A1 for a proof), or because they bring capture
little more information compared to existing accepted alternatives, they are
inefficient to compute and therefore not recommended to evaluate generative GNNs, e.g. the
Earth mover's distance-based kernel \citep{obray2022evaluation}.
...
data: the Weisfeiler-Lehman kernel. The second class of kernels leveraged here
...
the hashed value obtained from concatenating the node label of each neighbor
...
angles with their neighbor to make up the three-dimensional structure of the
...
allows one to capture the global structural features of the protein.
