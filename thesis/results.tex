...
\section{Overall \acrshort{mmd} Behavior}\label{sec:overall_behaviour}

\subsection{General observations on the correlation coefficients} Surprisingly,
...
\subsection{General observations on the standard deviations} In parallel, Table
...
\subsection{Influence of the choice of kernel} We next investigate the overall
...
\subsection{Comparing graph construction techniques from proteins} To compare which graph
...
\subsection{Lower values of $\varepsilon$ are more stable} In the previous paragraph, we established that $\varepsilon$-graphs were more
...
\subsection{Lower thresholds $\varepsilon$ for graph contruction are more
sensitive to smaller perturbations than higher $\varepsilon$ thresholds}
...
\subsection{Quality of MMD using Weisfeiler-Lehman} The first conclusion is
...
\subsection{No increase in normalized MMD in low regimes of
...
\section{\acrshort{mmd} from Learned Embeddings}\label{sec:insensitivity_wl_kernel}
...
case, highlighting the need for further evaluation.
...
\begin{table}
  ...
  \caption[$\sigma_\MMD$ values for the MMD using the ESM learned
embedding.]{$\sigma_\MMD$ values for the MMD using the ESM learned embedding.
