...
\begin{table}
  ...
  \begin{tabular}{lllr}
    \toprule
    &                              &    &  \textbf{Normalized MMD} \\
    Perturbation Type & Descriptor Function & $\varepsilon$ & \textbf{Standard Deviation} \\
    \midrule
...
\ref{fig:mmd_sensitivity_eps}, when 20\% of the maximum amount of perturbation
applied\footnote{See Table \ref{tab:perturbation_ranges} for the respective
maxima.}, in seven out of 12 cases, the normalized MMD 8\si{\angstrom} is higher
than that of the 16- or 32 \si{\angstrom}-graphs. In addition, we find that,
generally, in lower perturbation regimes, higher percentages of the normalized
MMD are reached with graphs constructed with a lower $\varepsilon$. Figure
\ref{fig:difference_sensitivity_20_percent} illustrates this phenomenon. By
grouping each configuration of MMD under different perturbation types at the
20\% mark, we see that the 8\si{\angstrom}- and 16\si{\angstrom} graphs are
higher than that of the 32\si{\angstrom} graphs (p-values).
...
\begin{figure}
  ...
  \caption{Normalized MMD value at 20\% of the maximum
    ...
    8\si{\angstrom}- and 16\si{\angstrom} graphs was not signicant ($p=3.47\cdot 10^{-1}$)}
