...
graph descriptors. We then move on to show the results of the sensitivity of the MMD
...
\section{Overall MMD behavior}

Surprisingly, we found that the behavior of MMD was not as inconsistent for the
...
The overall influence of the kernel on MMD's behavior subject to perturbations
...
We find that the kernel used does not have any bearing on the MMD behavior as
...
inter-run variance can be just as nefarious as MMD configurations with low
correlation coefficients because the range of absolute MMD values can be too wide
...
In this chapter, we present the results of the behavior of MMD under various
configurations and subject to various perturbations. We first note that the
overall behavior of MMD under various perturbations is stable when sufficient
...
choice of graph representation impacts the behavior of MMD's behavior: the
